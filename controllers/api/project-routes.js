...
router.get("/:username", (req, res) => {
  User.findOne({
    ...
    .then((data) => {
      if (!data) {
        Axios.get(
          `https://gh-pinned-repos.egoist.sh/?username=${req.params.username}`
        ).then((r) => {
          res.send(r.data);
        });
      }
...
router.post("/update", async (req, res) => {
  ...
  for (let i = 0; i < projectdata.length; i++) {
    statements.push(
      User.upsert()
      sequelize.query(
        `UPSERT INTO ${tableName} (owner, link, image, forks, language, languageColor, stars, repo) VALUES (
      '${projectdata[i].owner}', 
      '${projectdata[i].link}', 
      '${projectdata[i].image}',
      '${projectdata[i].forks}',
      '${projectdata[i].language}',
      '${projectdata[i].languageColor}', 
      '${projectdata[i].stars}',
      '${projectdata[i].repo}') 
      WHERE id=${i};`
      )
